...
class Report:
    ...
    def groupByPlot(self, group, measure):
        ...
        df2 = self.df
        #df2 = df2.dropna(axis="index", how = "any", subset=df2[group,measure].columns)
        Total = df2.groupby([group])[measure].sum().sort_values(ascending = False)
        mean = df2.groupby([group])[measure].mean().sort_values(ascending = False)
    ...
    # def removeNullRow(self,n): # remove rows containing more than n nan values
    #     df = self.df
